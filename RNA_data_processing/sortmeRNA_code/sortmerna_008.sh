...
find -L "$search_dir" -type f -name "*L008_R1_001.fastq.gz" | while read -r r1; do
    # Infer the R2 filename
    r2="${r1/L008_R1_001.fastq.gz/L008_R2_001.fastq.gz}"
    sample=$(basename "$r1" _L008_R1_001.fastq.gz)

    echo "===================== Running sortmeRNA on $sample ... ====================="

	sortmerna --threads 4 \
        --ref "$reference" \
	--workdir "$working_dir/008/3/18" \
	--reads "$r1" \
	--reads "$r2" \
	--fastx \
	--paired_in \
	...
	--aligned "$working_dir/aligned/${sample}_L008_rRNA" \
	--other "$working_dir/other/${sample}_L008_nonrRNA"

    # Emptying the intermediary files used for the analysis (we only need the output)
	rm -r "$working_dir/008/3/18/kvdb"
        rm -r "$working_dir/008/3/18/readb" 
