...
search_dir="/proj/naiss2024-6-73/Bianca/fastp/3/14"
...
find -L "$search_dir" -type f -name "*L006_R1_001.fastq.gz" | while read -r r1; do
    # Infer the R2 filename
    r2="${r1/L006_R1_001.fastq.gz/L006_R2_001.fastq.gz}"
    sample=$(basename "$r1" _L006_R1_001.fastq.gz)

    echo "===================== Running sortmeRNA on $sample ... ====================="

	sortmerna --threads 4 \
        --ref "$reference" \
	--workdir "$working_dir/006/3/14" \
	--reads "$r1" \
	--reads "$r2" \
	--fastx \
	--paired_in \
	...
	--aligned "$working_dir/aligned/${sample}_L006_rRNA" \
	--other "$working_dir/other/${sample}_L006_nonrRNA"

    # Emptying the intermediary files used for the analysis (we only need the output)
	rm -r "$working_dir/006/3/14/kvdb"
        rm -r "$working_dir/006/3/14/readb" 
